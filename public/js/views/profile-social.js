...
function activityItem(item) {
  if (item.kind === 'publication') {
    ...
    >
      ...
      <span
        ><small>Publicação</small><strong>${E(item.title)}</strong><em>${date(item.when)}</em></span
      >
    </a>`;
  }
  if (item.kind === 'project') {
    ...
    >
      <span class="profile-activity-icon">⌘</span>
      <span
        ><small>Projeto</small><strong>${E(item.title)}</strong><em>${date(item.when)}</em></span
      >
    </a>`;
  }
  return html`<a
    href="/coloquio/${encodeURIComponent(item.id)}"
    data-link
    class="profile-activity-item"
  >
    <span class="profile-activity-icon">◌</span>
    <span
      ><small>Discussão</small><strong>${E(item.title)}</strong><em>${date(item.when)}</em></span
    >
  </a>`;
}
...
function authorDetail(username) {
  ...
  layout(
    html`<section class="profile-social-page">
      <div class="shell">
          ...
          <div class="profile-social-main">
            <section id="atividade" class="profile-social-section">
              <div class="sectionhead">
                <div>
                  <div class="eyebrow">Recentia</div>
                  <h2>Atividade recente</h2>
                </div>
                <span class="profile-contribution-count"
                  >${stats.contributions} contribuições públicas</span
                >
              </div>
            ...
            <section id="projetos" class="profile-social-section">
              <div class="sectionhead">
                <div>
                  <div class="eyebrow">Proiecta</div>
                  <h2>Projetos</h2>
                </div>
              </div>
              <div class="grid grid2">
                ${stats.projects.map(projectCard).join('') ||
                empty('Nenhum projeto público ainda.')}
              </div>
            </section>
          ...
          <aside id="sobre" class="profile-social-sidebar">
            <section class="card profile-about-card">
              ...
              <dl>
                <div>
                  <dt>Perfil</dt>
                  <dd>${E(typeLabel(profile.profileType))}</dd>
                </div>
                <div>
                  <dt>Área</dt>
                  <dd>${E(profile.course || 'Não informada')}</dd>
                </div>
                <div>
                  <dt>Instituição</dt>
                  <dd>${E(profile.institution || 'Não informada')}</dd>
                </div>
                <div>
                  <dt>Formação</dt>
                  <dd>${E(profile.educationLevel || 'Não informada')}</dd>
                </div>
                <div>
                  <dt>Status</dt>
                  <dd>${profile.verificationStatus === 'verified' ? 'Verificado' : 'Membro'}</dd>
                </div>
              </dl>
            </section>
