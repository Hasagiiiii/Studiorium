...
function projectRow(project, published = false) {
  return html`<div class="project">
    <div class="min-width-zero">
      ...
      <p>${E(project.type)} · ${published ? 'público' : 'privado'} · atualizado ${date(project.updatedAt)}</p>
    </div>
    <div class="actions">
      ...
      <button class="dangerbtn" type="button" data-delete-project="${E(project.id)}">Excluir</button>
    </div>
  </div>`;
}
...
function codeRow(project, published = false) {
  return html`<div class="project">
    <div class="min-width-zero">
      <h3>${E(project.title)}</h3>
      <p>${published ? 'público' : E(project.visibility)} · atualizado ${date(project.updatedAt)}</p>
    </div>
    <div class="actions">
      ${link(`/laboratorio/${encodeURIComponent(project.id)}`, published ? 'Abrir e editar' : 'Codar', 'outline')}
      <button class="dangerbtn" type="button" data-delete-code-project="${E(project.id)}">Excluir</button>
    </div>
  </div>`;
}
...
function discussionRow(item, published = false) {
  return html`<div class="project">
    ...
    <div class="actions">
      ...
      <button class="dangerbtn" type="button" data-delete-own-discussion="${E(item.id)}">Excluir</button>
    </div>
  </div>`;
}
...
function enhanceProjects(me) {
  ...
  card.innerHTML = html`<div class="eyebrow">Projetos</div>
    <h3>Em sua mesa</h3>
    ${privateItems.map((item) => projectRow(item)).join('') || empty('Nenhum projeto privado na mesa.')}
...
function enhancePublications(me) {
  ...
  card.innerHTML = html`<div class="eyebrow">Publicações</div>
    ...
    ${working.map((item) => publicationRow(item)).join('') || empty('Nenhum trabalho em edição ou revisão.')}
...
function enhanceTech(me) {
  ...
  card.innerHTML = html`<div class="eyebrow">Oficina e tecnologia</div>
    <h3>Em edição e revisão</h3>
    ${working.map((item) => techRow(item)).join('') || empty('Nenhum conteúdo da Oficina em edição ou revisão.')}
...
function enhanceCode(me) {
  ...
  card.innerHTML = html`<div class="eyebrow">Projetos de código</div>
    ...
    ${privateItems.map((item) => codeRow(item)).join('') || empty('Nenhum projeto privado no laboratório.')}
  ...
  codeCard.insertAdjacentHTML(
    ...
    html`<div class="card" data-private-colloquium style="margin-top:18px">
      ...
      ${!pendingDiscussions.length && !pendingReplies.length
        ...
        : ''}
      ${publishedVault('Discussões publicadas', publicDiscussions, discussionRow, 'Nenhuma discussão publicada.')}
      ${publishedVault('Respostas publicadas', publicReplies, replyRow, 'Nenhuma resposta publicada.')}
      <div class="actions" style="margin-top:14px">
