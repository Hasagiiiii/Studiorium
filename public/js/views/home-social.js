...
function authorLine(item) {
  ...
  return html`<div class="social-author">
    ...
    <span><strong>${E(name)}</strong>${verified ? '<small class="social-verified">✓ Verificado</small>' : ''}<small>${date(item.createdAt || item.publishedAt || item.updatedAt)}</small></span>
  </div>`;
}
...
function publicationPost(p) {
  return html`<article class="social-post publication-card">
    ...
    ${p.coverName
      ? html`<a href="/pesquisas/${encodeURIComponent(p.slug)}" data-link class="social-media"><img src="/api/publications/${encodeURIComponent(p.id)}/cover" alt="Capa de ${E(p.title)}" loading="lazy" /></a>`
      : ''}
    <div class="social-tags">${(p.keywords || []).slice(0, 4).map((tag) => html`<span>${E(tag)}</span>`).join('')}</div>
    <div class="social-actions">
      <button class="social-action" type="button" data-publication-boost="${E(p.id)}">✦ <strong data-boost-count="${E(p.id)}">${num(p.boosts)}</strong> impulsos</button>
      ${link('/pesquisas/' + encodeURIComponent(p.slug), '◌ Abrir pesquisa', 'social-action')}
      <span class="social-stat">${num(p.views)} leituras</span>
    </div>
  </article>`;
}
...
function techPost(t) {
  return html`<article class="social-post project-card">
    ...
    <div class="social-actions">${link('/oficina/' + encodeURIComponent(t.slug), '⚙ Abrir tutorial', 'social-action')}</div>
  </article>`;
}
...
function trendingTopics() {
  ...
  (state.boot.publications || []).forEach((p) => {
    add(p.area);
    (p.keywords || []).forEach(add);
  });
  (state.boot.discussions || []).forEach((d) => add(d.category));
  (state.boot.techResources || []).forEach((t) => {
    add(t.category);
    (t.tags || []).forEach(add);
  });
...
function home() {
  ...
  const verified = (state.boot.profiles || []).filter((p) => p.verificationStatus === 'verified' || p.verificationStatus === 'approved').slice(0, 4);
    ...
    <section class="social-shell shell">
      <aside class="social-left">
        <div class="social-panel social-menu">
          <h3>Explorar</h3>
          ${link('/comunidades', '♧ Comunidades')}${link('/projetos', '⌘ Projetos')}${link('/biblioteca', '▤ Biblioteca')}${link('/coloquio', '◌ Discussões')}${link('/oficina', '⚙ Tecnologia')}${link('/autores', '◎ Pessoas')}${link('/noticias', '✦ Notícias')}
        </div>
        ...
        <div class="social-panel"><div class="social-panel-head"><h3>Assuntos em alta</h3>${link('/biblioteca', 'Pesquisar')}</div><div class="social-topic-cloud">${topics.length ? topics.map((topic) => html`<a href="/biblioteca?q=${encodeURIComponent(topic)}" data-link>#${E(topic)}</a>`).join('') : '<span class="muted small">Os assuntos aparecem conforme a comunidade publica.</span>'}</div></div>
      </aside>
      ...
      <main class="social-feed">
        ...
        ${state.me
          ? html`<div class="social-composer"><span class="social-avatar">${E((state.me.displayName || 'S').slice(0, 1))}</span><div><strong>Compartilhe algo com a comunidade</strong><p>Publique uma pesquisa, abra uma discussão ou mostre um projeto.</p></div><div class="actions">${link('/publicar', 'Publicar', 'solid')}${link('/comunidades', 'Discutir', 'soft')}</div></div>`
          : html`<div class="social-composer guest"><div><strong>Entre para fazer parte da conversa.</strong><p>Siga comunidades, publique conhecimento e participe das discussões.</p></div><div class="actions">${link('/cadastro', 'Criar conta', 'solid')}${link('/login', 'Entrar', 'outline')}</div></div>`}
      ...
      <aside class="social-right">
        ...
        <div class="social-panel"><div class="social-panel-head"><h3>Leituras da comunidade</h3>${link('/biblioteca', 'Biblioteca')}</div>${books.length ? books.map((book) => html`<a class="social-list-item" href="/livros/${encodeURIComponent(book.id)}" data-link><span class="project-mini">▤</span><span><strong>${E(book.title)}</strong><small>${E(book.author || book.category || 'Livro recomendado')}</small></span></a>`).join('') : '<p class="muted">As recomendações de livros aparecerão aqui.</p>'}</div>
        <div class="social-panel"><div class="social-panel-head"><h3>Especialistas verificados</h3>${link('/autores', 'Pessoas')}</div>${(verified.length ? verified : profiles).map((p) => html`<a class="social-list-item" href="/autores/${encodeURIComponent(p.username)}" data-link><span class="social-avatar">${E((p.displayName || p.username || 'S').slice(0, 1))}</span><span><strong>${E(p.displayName || p.username)}</strong><small>${p.verificationStatus === 'verified' || p.verificationStatus === 'approved' ? '✓ ' : ''}${E(p.verifiedSpecialty || p.profileType || 'Membro')}</small></span></a>`).join('')}</div>
