        ...
        <section className="profile-content-section profile-post-grid" aria-label="Publicações">
          {detail.posts.length ? (
            detail.posts.map((post) => (
              <SocialPostCard key={post.id} post={post} compact showAuthor={false} />
            ))
          ) : (
            <div className="empty-state">
              <h2>Nenhuma publicação ainda.</h2>
              <p>As publicações visíveis deste perfil aparecerão aqui.</p>
            </div>
          )}
        </section>
      ) : null}
              ...
              detail.communities.map((community) => (
                <article className="resource-card" key={community.id}>
                  <span className="eyebrow">
                    {community.official ? 'Comunidade oficial' : 'Comunidade'}
                  </span>
                  <h2>
                  ...
                  <p>
                    {community.area} · {community.role}
                  </p>
                </article>
              ))
            ) : (
              <div className="empty-state">
                <h2>Nenhuma comunidade visível.</h2>
              </div>
            )}
          </div>
        </section>
      ...
      {active === 'books' ? (
        <section className="profile-content-section profile-bookshelf-section">
          <div className="profile-bookshelf-heading">
            <div>
              ...
              <p>
                ...
                {detail.bookshelf.length === 1 ? 'livro salvo' : 'livros salvos'}
              </p>
            </div>
            {detail.isOwnProfile ? (
              <label>
          ...
          {!detail.isOwnProfile && !profile.bookshelfPublic ? (
            ...
            <div className="empty-state">
              <h2>A estante ainda está vazia.</h2>
            </div>
          )}
        </section>
      ) : null}
