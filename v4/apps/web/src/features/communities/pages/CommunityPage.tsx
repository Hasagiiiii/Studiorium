...
export function CommunityPage() {
  ...
  const [requestsStatus, setRequestsStatus] = useState<'idle' | 'loading' | 'ready' | 'error'>('idle');
  const [requestsError, setRequestsError] = useState('');
  const [decidingUserId, setDecidingUserId] = useState('');

  useEffect(() => {
    const canModerate = community?.joined && (community.role === 'leader' || community.role === 'moderator');
    if (!canModerate) {
      setRequests([]);
    ...
    setRequestsError('');
    void services.communities.requests(community.slug).then((value) => {
      if (!active) return;
      setRequests(value);
      ...
      setRequestsError(cause instanceof Error ? cause.message : 'Não foi possível carregar as solicitações.');
    });
  ...
  if (!community) {
    return (
      <FeaturePage eyebrow="Comunidades" title="Comunidade não encontrada" description="Ela pode ser privada, ter sido removida ou o endereço pode estar incorreto.">
  ...
  const moderationStatus = membership?.memberModerationStatus ?? currentCommunity.memberModerationStatus;
  ...
  const visibilityLabel = currentCommunity.visibility === 'public'
    ? 'Pública'
  ...
  async function updateMembership(action: 'join' | 'leave' | 'request') {
    ...
    try {
      ...
      const message = cause instanceof Error ? cause.message : 'Não foi possível atualizar sua participação.';
  ...
  async function decideRequest(userId: string, approve: boolean) {
    ...
    try {
      ...
      pushToast({ message: approve ? 'Solicitação aprovada.' : 'Solicitação rejeitada.', tone: 'success' });
    } catch (cause) {
      const message = cause instanceof Error ? cause.message : 'Não foi possível decidir a solicitação.';
    ...
    <div className="community-detail-page">
      <section className="community-hero" aria-labelledby="community-title">
        ...
        <div className="community-identity-row">
          <div className="community-avatar" aria-hidden="true">{currentCommunity.name.slice(0, 1).toUpperCase()}</div>
          <div className="community-identity-copy">
            <div className="community-title-line">
              <h1 id="community-title">{currentCommunity.name}</h1>
              {currentCommunity.official ? <span className="community-official">Oficial</span> : null}
            </div>
          ...
          <div className="community-primary-action">
            {moderationStatus === 'removed' ? (
              ...
              role === 'leader' ? <span className="community-membership-state">Você lidera esta comunidade</span> : (
                <button className="button secondary" type="button" disabled={updating} onClick={() => void updateMembership('leave')}>
                  {updating ? 'Atualizando…' : 'Participando'}
                </button>
              )
            ) : !data?.user ? (
              <Link className="button primary" to={`/entrar?retorno=${encodeURIComponent(`/comunidades/${currentCommunity.slug}`)}`}>Entrar para participar</Link>
            ) : currentCommunity.visibility === 'public' ? (
              <button className="button primary" type="button" disabled={updating} onClick={() => void updateMembership('join')}>
                {updating ? 'Entrando…' : 'Participar'}
              </button>
            ) : currentCommunity.visibility === 'restricted' ? (
              membershipStatus === 'pending' ? <span className="community-membership-state">Solicitação enviada</span> : (
                <button className="button primary" type="button" disabled={updating} onClick={() => void updateMembership('request')}>
                  {updating ? 'Enviando…' : membershipStatus === 'rejected' ? 'Solicitar novamente' : 'Solicitar entrada'}
                </button>
              )
        ...
        <nav className="community-local-nav" aria-label="Navegação da comunidade">
          ...
          {canModerate ? <a href="#moderacao">Moderação{requests.length ? ` (${requests.length})` : ''}</a> : null}
        </nav>
      </section>

      {membershipError ? <p className="inline-error community-page-error" role="alert">{membershipError}</p> : null}
      ...
      <div className="community-interior-layout">
        ...
        <aside className="community-interior-sidebar">
          <section id="sobre" className="community-info-panel">
            ...
            <p>{currentCommunity.description || 'Comunidade criada para reunir pessoas, ideias e conteúdo em torno deste tema.'}</p>
            <dl className="community-facts">
              <div><dt>Área</dt><dd>{currentCommunity.area}</dd></div>
              <div><dt>Membros</dt><dd>{memberCount}</dd></div>
              <div><dt>Acesso</dt><dd>{visibilityLabel}</dd></div>
            </dl>
          </section>
          ...
          {currentCommunity.rules.length ? (
            <section id="regras" className="community-info-panel">
              ...
              <ol className="community-rules-list">
                {currentCommunity.rules.map((rule) => <li key={rule}>{rule}</li>)}
              </ol>
            </section>
          ) : null}
          ...
          {canModerate ? (
            <section id="moderacao" className="community-info-panel community-moderation-panel">
              ...
              {requestsStatus === 'ready' && !requests.length ? <p>Nenhuma solicitação pendente.</p> : null}
              {requests.length ? (
                <ul className="community-request-list">
                  {requests.map((request) => (
                    <li key={request.userId}>
                      <div>
                        {request.username ? <Link to={`/perfil/${encodeURIComponent(request.username)}`}>{request.displayName}</Link> : <strong>{request.displayName}</strong>}
                        {request.username ? <small>@{request.username}</small> : null}
                      </div>
                      <div className="community-request-actions">
                        <button type="button" disabled={Boolean(decidingUserId)} onClick={() => void decideRequest(request.userId, true)}>Aprovar</button>
                        <button type="button" disabled={Boolean(decidingUserId)} onClick={() => void decideRequest(request.userId, false)}>Recusar</button>
                      </div>
                    </li>
                  ))}
                </ul>
              ) : null}
              {requestsStatus === 'error' || requestsError ? <p className="inline-error" role="alert">{requestsError || 'Não foi possível carregar as solicitações.'}</p> : null}
            </section>
          ) : null}
        </aside>
      </div>
