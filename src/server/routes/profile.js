...
const TYPES = ['estudante','universitario','professor','pesquisador','designer','instituicao','criador'];

async function updateProfile(req) {
  const user = await requireUser(req);
  const body = await readJson(req);
  const { data: current, error } = await db().from('profiles').select('*').eq('user_id', user.id).maybeSingle();
  fail(error);
  if (!current) throw Object.assign(new Error('Perfil não encontrado.'), { statusCode: 404 });
  const patch = {
    display_name: String(body.displayName || current.display_name).trim().slice(0, 80) || current.display_name,
