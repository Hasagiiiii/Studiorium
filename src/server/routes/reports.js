...
async function targetExists(type, targetId) {
  const map = {
    discussion: ['discussions', 'status'],
    reply: ['replies', 'status'],
...
async function createReport(req) {
  ...
  if (!categories.includes(body.category))
    throw Object.assign(new Error('Categoria de denúncia inválida.'), { statusCode: 400 });
  const targetType = String(body.targetType || '').slice(0, 40);
  const targetId = String(body.targetId || '').slice(0, 100);
  if (!targetType || !targetId)
    throw Object.assign(new Error('Alvo da denúncia ausente.'), { statusCode: 400 });
  if (!(await targetExists(targetType, targetId)))
    throw Object.assign(
      new Error('O conteúdo denunciado não foi encontrado ou não está público.'),
      { statusCode: 404 },
    );
  ...
  fail(duplicateError);
  if (duplicate)
    throw Object.assign(new Error('Você já possui uma denúncia aberta para este conteúdo.'), {
      statusCode: 409,
    });
  const row = {
    id: id('rep'),
    ...
    description: String(body.description || '')
      ...
      .slice(0, 1500),
    status: 'open',
    priority: ['risco_menor', 'conteudo_sexual'].includes(body.category) ? 'urgent' : 'normal',
    created_at: now(),
    updated_at: now(),
  };
