...
const settingKeys = new Set(['site_title','hero_title','hero_text','site_notice','registrations_open','maintenance_mode']);
...
async function enrichReports(rows) {
  ...
  for (const row of rows) if (grouped[row.target_type]) grouped[row.target_type].push(row.target_id);
  const [pubsQ, discussionsQ, repliesQ] = await Promise.all([
    grouped.publication.length ? db().from('publications').select('id,title,abstract,status,author_name').in('id', grouped.publication) : Promise.resolve({ data: [], error: null }),
    grouped.discussion.length ? db().from('discussions').select('id,title,body,status,author_name').in('id', grouped.discussion) : Promise.resolve({ data: [], error: null }),
    grouped.reply.length ? db().from('replies').select('id,body,status,author_name,discussion_id').in('id', grouped.reply) : Promise.resolve({ data: [], error: null }),
  ]);
  [pubsQ, discussionsQ, repliesQ].forEach((q) => fail(q.error));
  const maps = {
    publication: new Map(pubsQ.data.map((x) => [x.id, { title:x.title, excerpt:x.abstract, status:x.status, authorName:x.author_name }])),
    discussion: new Map(discussionsQ.data.map((x) => [x.id, { title:x.title, excerpt:x.body, status:x.status, authorName:x.author_name }])),
    reply: new Map(repliesQ.data.map((x) => [x.id, { title:'Resposta no Colóquio', excerpt:x.body, status:x.status, authorName:x.author_name, discussionId:x.discussion_id }])),
  };
  return rows.map((row) => ({ ...S.report(row), target: maps[row.target_type]?.get(row.target_id) || null }));
}

async function dashboard(req) {
  await requireAdmin(req);
  const [usersQ, profilesQ, publicationsQ, discussionsQ, reportsQ, templatesQ, settingsQ, auditQ] = await Promise.all([
    ...
    db().from('templates').select('*').order('featured', { ascending: false }).order('downloads', { ascending: false }),
    db().from('site_settings').select('*'),
    db().from('admin_audit_log').select('*').order('created_at', { ascending: false }).limit(100),
  ]);
  [usersQ, profilesQ, publicationsQ, discussionsQ, reportsQ, templatesQ, settingsQ, auditQ].forEach((q) => fail(q.error));
  ...
  const metrics = {
    ...
    urgentReports: reports.filter((r) => ['open','reviewing'].includes(r.status) && r.priority === 'urgent').length,
    discussions: discussions.filter((d) => d.status === 'published').length,
    templates: templates.length,
  };
  return {
    metrics, users, publications, discussions, reports, templates,
    settings: settingsObject(settingsQ.data),
    audit: auditQ.data.map((row) => ({
      id: row.id, adminId: row.admin_id, action: row.action, targetType: row.target_type,
...
async function queue(req) {
  ...
  const [reportsQ, publicationsQ] = await Promise.all([
    db().from('reports').select('*').in('status', ['open','reviewing']).order('created_at', { ascending: false }),
    db().from('publications').select('*').eq('status', 'pending_review').order('created_at', { ascending: false }),
  ]);
  fail(reportsQ.error); fail(publicationsQ.error);
  const reports = (await enrichReports(reportsQ.data)).sort((a, b) => (a.priority === 'urgent' ? 0 : 1) - (b.priority === 'urgent' ? 0 : 1) || b.createdAt.localeCompare(a.createdAt));
...
async function updateReport(req, reportId) {
  ...
  if (['open','reviewing','resolved','dismissed'].includes(body.status)) patch.status = body.status;
  ...
  const { data, error } = await db().from('reports').update(patch).eq('id', reportId).select('*').maybeSingle();
  fail(error);
...
async function updatePublication(req, publicationId) {
  ...
  const { data, error } = await db().from('publications').update(patch).eq('id', publicationId).select('*').maybeSingle();
  fail(error);
...
async function updateContent(req, type, targetId) {
  ...
  if (!c.allowed.includes(body.status)) throw Object.assign(new Error('Status inválido.'), { statusCode: 400 });
  const patch = { status: body.status };
  if (type === 'publication' && body.status === 'published') patch.published_at = now();
  const { data, error } = await db().from(c.table).update(patch).eq('id', targetId).select('*').maybeSingle();
  fail(error);
...
async function updateUser(req, userId) {
  const admin = await requireAdmin(req);
  const body = await readJson(req);
  const { data: target, error: targetError } = await db().from('users').select('*').eq('id', userId).maybeSingle();
  fail(targetError);
  ...
  if (body.role && ['user','admin'].includes(body.role)) {
    if (protectedAdmin && body.role !== 'admin') throw Object.assign(new Error('A conta administradora principal não pode perder a função de ADM.'), { statusCode: 400 });
    if (target.id === admin.id && body.role !== 'admin') throw Object.assign(new Error('Você não pode remover sua própria permissão de ADM.'), { statusCode: 400 });
    patch.role = body.role;
  }
  if (body.status && ['active','suspended'].includes(body.status)) {
    if ((protectedAdmin || target.id === admin.id) && body.status === 'suspended') throw Object.assign(new Error('Essa conta administrativa não pode ser suspensa por esta ação.'), { statusCode: 400 });
    patch.status = body.status;
    patch.suspension_reason = body.status === 'suspended' ? String(body.reason || '').trim().slice(0, 500) : '';
    patch.suspended_at = body.status === 'suspended' ? now() : null;
  }
  if (!Object.keys(patch).length) throw Object.assign(new Error('Nenhuma alteração válida informada.'), { statusCode: 400 });
  ...
  await audit(admin, 'user.update', 'user', userId, { role:patch.role, status:patch.status, reason:patch.suspension_reason });
...
async function updateTemplate(req, templateId) {
  ...
  if (typeof body.category === 'string' && body.category.trim()) patch.category = body.category.trim().slice(0, 80);
  if (typeof body.docType === 'string' && body.docType.trim()) patch.doc_type = body.docType.trim().slice(0, 80);
  if (typeof body.style === 'string' && body.style.trim()) patch.style = body.style.trim().slice(0, 80);
  if (typeof body.description === 'string') patch.description = body.description.trim().slice(0, 2000);
  if (typeof body.featured === 'boolean') patch.featured = body.featured;
  if (!Object.keys(patch).length) throw Object.assign(new Error('Nenhuma alteração válida informada.'), { statusCode: 400 });
  const { data, error } = await db().from('templates').update(patch).eq('id', templateId).select('*').maybeSingle();
  fail(error);
...
async function updateSettings(req) {
  ...
  for (const [key, value] of Object.entries(body || {})) {
    ...
    else clean = String(value ?? '').trim().slice(0, key === 'hero_text' ? 1200 : 300);
    rows.push({ key, value: clean, updated_at: now() });
  }
  if (!rows.length) throw Object.assign(new Error('Nenhuma configuração válida informada.'), { statusCode: 400 });
  const { data, error } = await db().from('site_settings').upsert(rows, { onConflict:'key' }).select('*');
  fail(error);
  await audit(admin, 'settings.update', 'system', 'site_settings', Object.fromEntries(rows.map((r) => [r.key, r.value])));
  return { settings: settingsObject(data) };
}

module.exports = { dashboard, queue, updateReport, updatePublication, updateContent, updateUser, updateTemplate, updateSettings };
