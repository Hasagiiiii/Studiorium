...
async function register(req, res) {
  const { data: registrationSetting } = await db()
    .from('site_settings')
    .select('value')
    .eq('key', 'registrations_open')
    .maybeSingle();
  if (registrationSetting && registrationSetting.value === false)
    throw Object.assign(new Error('Novos cadastros estão temporariamente pausados.'), {
      statusCode: 403,
    });
  ...
  if (password.length < 8 || password.length > 128)
    throw Object.assign(new Error('A senha precisa ter entre 8 e 128 caracteres.'), {
      statusCode: 400,
    });
  const year = new Date().getFullYear();
  if (!Number.isInteger(birthYear) || birthYear < 1930 || birthYear > year)
    throw Object.assign(new Error('Ano de nascimento inválido.'), { statusCode: 400 });
  if (displayName.length < 2)
    throw Object.assign(new Error('Informe seu nome de exibição.'), { statusCode: 400 });
  ...
  if (config().adminEmail && email === config().adminEmail) {
    ...
    throw Object.assign(
      new Error('A conta administradora principal já é provisionada pelo sistema.'),
      { statusCode: 403 },
    );
  }
  ...
  return {
    status: 201,
    body: {
      user: await publicUser({ id: userId, email, role, is_minor: isMinor, created_at: now() }),
    },
  };
}
...
async function login(req, res) {
  ...
  if (!user || !verifyPassword(String(body.password || ''), user.password_hash)) {
    const limit = await recordAuthFailure(req, 'login', email, {
      maxAttempts: 8,
      windowMs: 15 * 60 * 1000,
      blockMs: 15 * 60 * 1000,
    });
    ...
    if (limit.blocked) {
      throw Object.assign(
        new Error('Muitas tentativas. Aguarde alguns minutos e tente novamente.'),
        { statusCode: 429 },
      );
    }
  ...
  if ((user.status || 'active') === 'suspended') {
    ...
    throw Object.assign(
      new Error('Esta conta está suspensa. Entre em contato com a administração.'),
      { statusCode: 403 },
    );
  }
  await clearAuthFailures(req, 'login', email);
  await createSession(user.id, res);
  await logSecurityEvent(req, 'auth.login_success', {
    userId: user.id,
    email,
...
async function changePassword(req, res) {
  ...
  if (!account || !verifyPassword(currentPassword, account.password_hash)) {
    await logSecurityEvent(req, 'auth.password_change_failed', {
      userId: user.id,
      email: user.email,
    });
