...
async function bootstrap(req) {
  const client = db();
  const [templatesQ, publicationsQ, discussionsQ, profilesQ, settingsQ, techQ, codeQ, user] =
    await Promise.all([
      client
        .from('templates')
        .select('*')
  ...
  [templatesQ, publicationsQ, discussionsQ, profilesQ, settingsQ, techQ, codeQ].forEach((q) =>
    fail(q.error),
  );
